""" derived.py - module for calculating derived fields 
    by jkrasting@noaa.gov
    adapted to MOM5 by rfarneti@ictp.it
...
import numpy as np
import xarray as xr
#from momsofia import spice
from momsofia import util
from momsofia import eos_wright
from gsw import SA_from_SP, p_from_z, sigma2, CT_from_t, sigma0, sigma1, sigma4
...
def adjust_negative_n2(n2, zcoord="st_ocean"):
    ...
    print('--- Removing negative values of the buoyancy frequency')
...
def calc_alpha(thetao, so, pres, eos="Wright"):
    ...
    ---> Could also use gsw.density.alpha(SA, CT, p)
    ...
    #    dask="forbidden",
    #)
    alpha = eos_wright.alpha(thetao,so,pres)   
    alpha.attrs = {
...
def calc_beta(thetao, so, pres, eos="Wright"):
    ...
    ---> Could also use gsw.density.beta(SA, CT, p)
    ...
    #    dask="forbidden",
    #)
    beta = eos_wright.beta(thetao,so,pres)   
    beta.attrs = {
...
def calc_rel_vort_MOM5(ds):
    """Function to calculate relative vorticity

    This function calculates the vertical component of the relative vorticity
    from the u and v components of the flow.

    Using xgcm we can replicate the MOM5 vertical vorticity computation
    
    Parameters
    ----------
    ds : xarray.core.dataset.Dataset
        Input dataset. Required variables are `u`, `v`, `dxu`, `dyt`,
    ...
    # xu_ocean is shifted to the right of xt_ocean by 1/2 grid-cell. 
    ...
    relvort = relvort.rename("Relative Vorticity")
    relvort.attrs = {
...
def calc_volume(area, dz):
    """Function to calculate the ocean volume

    This function calculates the ocean volume by multiplying the
    area of the cell by the thickness of the cell 
    and applying appropriate CF metadata.

    Parameters
    ----------
    area : xarray.core.dataarray.DataArray
        The (tracer or velocity) cell area in units of m^2
    dht : xarray.core.dataarray.DataArray
        The cell thickness in units of m
    ...
    volume = dz * area
    volume.attrs = {
        "standard_name": "volume_of_water_mass",
        "long_name": "Volume of tracer cell",
        "units": "m^3",
    }
    return volume

def calc_mass(rho, volume):
    """Function to calculate the total ocean mass

    This function calculates the ocean mass by multiplying the
    in situ density by cell volume
    and applying appropriate CF metadata.
    ...
    rho : xarray.core.dataarray.DataArray
        The in situ density field in units of kg m-3
    volume : xarray.core.dataarray.DataArray
        The volcello field in units of m3
...
def calc_pv(
    ...
    coord_dict : :obj:`dict`, optional
        Dictionary of xgcm coordinate name mappings, if different from
        the MOM6 default values, by default None
    interp_f : bool, optional
        Interpolate Coriolis parameter to the corner points. Set to `False` to calculate
    ...
    full : bool, optional
        choice on how to compute PV. `True` uses relative vorticity, otherwise
        compute the `buoyancy` PV with coriolis only. By default False 
    Returns
    -------
    xarray.core.dataarray.DataArray
    ...
    if interp_f is True:
        ...
        # interpolate Coriolis to the corner points
        coriolis = grid.interp(coriolis, axis=["X", "Y"], boundary="fill")
        # or perform a dirty trick
        #coriolis.coords['xu_ocean'] = n2.xt_ocean.values
        #coriolis.coords['yu_ocean'] = n2.yt_ocean.values
        #coriolis = coriolis.rename({'xu_ocean': 'xt_ocean', 'yu_ocean': 'yt_ocean'})
    #print('zeta = ', zeta.dims)
    #print('N2 = ', n2.dims)
    #print('Coriolis = ', coriolis.dims) 
    
    if full is True:
       potvort = (zeta + coriolis) * (n2 / gravity)
    else:
       potvort = (n2 / gravity) * coriolis
    ...
    if units == "m":
        ...
        potvort = np.abs(((potvort / 100) * 1.0e14))
        potvort.attrs = {
            "long_name": "Ocean potential vorticity",
...
def calc_rho(thetao, so, pres, eos="Wright"):
    ...
    #    dask="allowed",
    #)
    rho = eos_wright.density(thetao,so,pres)   
...
def calc_pot_rho(salt, temp, depth, lat, lon, sigma):
    '''Function to calculate potential density 
       
       Potential density can be referenced to 
       0, 2000, or 4000 dbar 
       ...
       thetao
       so
       sigma : can be sigma0, sigma2, or sigma4 
       First, it needs to compute:
    ...
    if sigma=='sigma0':
       print('--- computing pot_rho_0')
       pot_rho = sigma0(abs_salt,temp)
    if sigma=='sigma1':
       print('--- computing pot_rho_1')
       pot_rho = sigma1(abs_salt,temp)
    elif sigma=='sigma2':
       print('--- computing pot_rho_2')
       pot_rho = sigma2(abs_salt,temp)
    elif sigma=='sigma4':
       print('--- computing pot_rho_4')
       pot_rho = sigma4(abs_salt,temp)
    else:
       print('wrong sigma!')
       exit()
   
    pot_rho = pot_rho.rename("pot_rho_") 
    pot_rho.attrs = {
